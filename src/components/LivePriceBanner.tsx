...
export const LivePriceBanner: React.FC<LivePriceBannerProps> = ({ currentTick, recentCandles }) => {
  ...
  return (
    <div className="bg-[#071426]/95 border-b border-[#00E5FF]/20 px-2 sm:px-4 py-2.5 sm:py-3.5 shadow-xs backdrop-blur-xl text-slate-100 relative overflow-hidden font-sans w-full min-w-0">
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3 sm:gap-4 relative z-10 w-full min-w-0">
        
        {/* Left: Ticker Symbol & Centerpiece Main Market Price */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2.5 sm:gap-5 w-full md:w-auto justify-between md:justify-start min-w-0">
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-[#00E5FF]/10 border border-[#00E5FF]/30 flex items-center justify-center text-[#00E5FF] shrink-0 shadow-[0_0_12px_rgba(0,229,255,0.15)]">
              <Layers className="w-4 h-4 sm:w-5 sm:h-5 text-[#00E5FF] animate-pulse" />
            </div>
            <div>
              <div className="flex items-center gap-1.5 sm:gap-2">
                <span className="text-sm sm:text-lg font-extrabold tracking-wider text-white font-mono whitespace-nowrap">
                  XAU/USD
                </span>
                <span className="text-[9px] sm:text-[10px] bg-[#00E5FF]/10 text-[#00E5FF] border border-[#00E5FF]/30 px-1.5 sm:px-2 py-0.5 rounded-full font-mono font-bold uppercase tracking-wider whitespace-nowrap">
                  SPOT GOLD
                </span>
              </div>
              <span className="text-[10px] sm:text-[11px] text-slate-400 font-mono flex items-center gap-1 mt-0.5 whitespace-nowrap">
          ...
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-3">
            <div
              className={`text-sm sm:text-xl lg:text-2xl font-extrabold font-mono tracking-tight transition-all duration-300 px-2 sm:px-3 py-1 rounded-lg sm:rounded-2xl ${
                tickDirection === 'UP'
            ...
            <div
              className={`flex items-center gap-0.5 sm:gap-1 font-mono text-[10px] sm:text-sm font-bold px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl border transition-all ${
                isUp
                  ? 'bg-[#22C55E]/10 border-[#22C55E]/30 text-[#22C55E] shadow-xs'
                  : 'bg-[#EF4444]/10 border-[#EF4444]/30 text-[#EF4444] shadow-xs'
              }`}
            >
              {isUp ? <ArrowUpRight className="w-3.5 h-3.5 text-[#22C55E] shrink-0" /> : <ArrowDownRight className="w-3.5 h-3.5 text-[#EF4444] shrink-0" />}
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-4 font-mono w-full md:w-auto justify-between md:justify-end min-w-0">
          
          {/* 3 Identical Cards: SPREAD, BID, ASK — Adapts to 3 columns on sm/md and grid on tiny screens */}
          <div className="grid grid-cols-3 gap-1.5 sm:gap-3 w-full sm:w-auto">
            
            {/* 1. SPREAD Card */}
            <div className="bg-[#030B16]/80 border border-[#00E5FF]/20 p-1.5 sm:p-3 rounded-xl sm:rounded-[20px] shadow-xs text-center hover:border-[#00E5FF]/50 transition-colors min-w-0">
              <span className="text-[8px] sm:text-[10px] text-slate-400 uppercase tracking-wider font-extrabold block mb-0.5 truncate">
                SPREAD
              </span>
              <span className="text-[11px] sm:text-sm font-black text-[#00E5FF] block font-mono truncate">
                ${currentTick.spread.toFixed(2)}
              </span>
            </div>

            {/* 2. BID Card */}
            <div className="bg-[#030B16]/80 border border-[#00E5FF]/20 p-1.5 sm:p-3 rounded-xl sm:rounded-[20px] shadow-xs text-center hover:border-[#22C55E]/50 transition-colors min-w-0">
              <span className="text-[8px] sm:text-[10px] text-slate-400 uppercase tracking-wider font-extrabold block mb-0.5 truncate">
                BID<span className="hidden sm:inline"> (OFFRE)</span>
              </span>
              <span className="text-[11px] sm:text-sm font-black text-[#22C55E] block font-mono truncate">
                ${currentTick.bid.toFixed(2)}
              </span>
            </div>

            {/* 3. ASK Card */}
            <div className="bg-[#030B16]/80 border border-[#00E5FF]/20 p-1.5 sm:p-3 rounded-xl sm:rounded-[20px] shadow-xs text-center hover:border-[#EF4444]/50 transition-colors min-w-0">
              <span className="text-[8px] sm:text-[10px] text-slate-400 uppercase tracking-wider font-extrabold block mb-0.5 truncate">
                ASK<span className="hidden sm:inline"> (DEMANDE)</span>
              </span>
              <span className="text-[11px] sm:text-sm font-black text-[#EF4444] block font-mono truncate">
